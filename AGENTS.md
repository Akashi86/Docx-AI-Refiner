...
- UI 新增“重复降低 AI 率轮数”，范围 `1-3`，默认 `1`。
- `1` 表示只走常规改写和风险触发的自动二次改写。
- `2/3` 会将上一轮改写结果继续送入 DeepSeek 再改一轮或两轮，用时间和 API 调用换更强处理。
- 重复轮次使用 `apply_repeat_ai_reduction()`，输入是上一轮结果，不是原文；提示词要求保留事实、引用、术语、数字和专名，同时继续改变句式节奏，避免恢复模板化论文腔。
- `rewrite_rounds` 会累计普通改写、风险二次改写和重复降 AI 轮次；`second_rewrite_applied` 在风险二次改写或重复轮次生效时为 `True`。
...
- 修订痕迹版改为可选生成，默认关闭。
- 新增重复降低 AI 率轮数，支持对上一轮改写结果继续进行 1-3 轮处理。
