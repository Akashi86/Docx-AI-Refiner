...
**整段高成功率改写 + 段落风险扫描 + 高风险段落定向二次改写 + AIGC 检测报告定点返修 + 改写对照清单 + 修订痕迹版下载。**
...
当前默认英文降 AI 模板改为用户给定的“专业 AIGC 降率工具”策略，核心强调：

- 重点目标是降低 AIGC 率，而不是普通润色。
- 可以调整句式，也可以适当口语化，但不能丢失专业性。
- 所有专有名词、技术术语、人名、题名、引用、数字、数据必须完整保留。
- 遇到专有名词或术语很多、不好调整的句子，直接简化、压缩长度。
- 遇到专有名词或术语较少的句子，可以适度扩展内容，把语句写详细。
- 在中心思想不变、专有名词不变的前提下，使用同义表达替换、调整句首和从句顺序。
- 不新增无依据事实、数据、引文或结论。
- 题名、关键词、参考文献、图表标题、列表标签仍应原样返回。
...
- UI 已将“生成修订痕迹版”改为可选项，默认关闭，以免 `docx_editor` 对比阶段拖慢主下载。
- 如果用户勾选后修订痕迹生成失败，会保留普通润色版下载，不阻塞主流程。
- 检测报告定点返修模式下，只有勾选“生成修订痕迹版（较慢）”时才展示“上传第一版/原始 Word”入口；未勾选时只输出返修版 Word。
- UI 新增独立处理模式“生成修订文档”：用户上传原文 Word 和修改后 Word，应用直接调用 `generate_tracked_changes_docx()` / `docx_editor` 生成修订痕迹版，不需要 DeepSeek 或百度翻译 API。
...
- 最新逻辑中修订痕迹版默认跳过，只有用户勾选后才会进入最后的修订生成阶段。

### 9.1 中文中转回译

- UI 新增“启用中文中转回译”，默认关闭。
- 开启后，每段会先走常规改写和风险触发的二次改写，再执行“英文结果 -> 中文中间稿 -> 英文最终稿”。
- 回译使用 `apply_back_translation_reduction()` 和 `call_deepseek_direct()`，不会套用普通 `make_system_prompt()`，因为普通提示词要求输出语言与输入一致，会与翻译目标冲突。
- 回译提示词要求保留引用、数字、人名、书名、理论名、语料库术语和技术术语，不新增事实，不输出 Markdown。
- 回译会显著增加耗时和 API 调用次数，每个启用回译的段落额外调用两次模型。
- `rewrite_rounds` 会累计普通改写、风险二次改写和回译两次调用；`second_rewrite_applied` 在风险二次改写或回译生效时为 `True`。

### 9.2 格式安全阀

- UI 新增“启用格式安全阀”，默认开启。
- 该开关目前控制 `markdown_formatting_added`：当原文没有星号、模型输出新增 `*` 或 `**` 这类 Markdown 痕迹时，开启状态会拒绝写回。
- 关闭格式安全阀后，只跳过 Markdown 星号拦截；标题保护、保护性文本改变、短标题扩写、过长扩写、口语化改写器词组等内容安全阀仍然生效。
...
1. 用户上传 Word `.docx`。
2. 用户上传 AIGC 检测报告 HTML/HTM/PDF。
3. 用户可选上传第一版/原始 Word，用于生成“原文 -> 最终返修版”的修订痕迹版。
4. 程序从报告中提取疑似 AI 片段。
   - HTML 优先，使用内置解析器提取旧版 `cl1/cl2/cl3` 高亮片段、`hide_3` 汇总片段，以及 PaperYY 新版 `em.high` / `em.medium` / `em.low` 疑似片段。
   - PDF 使用 `pypdf` 做文本提取，效果取决于 PDF 文本层质量。
5. 程序扫描 Word 正文任务段落。
6. 使用归一化文本匹配，把报告片段映射回 Word 段落。
7. 只把命中的段落送 DeepSeek 改写。
8. 输出 `返修版_原文件名.docx`、修订痕迹版、以及带匹配信息的 `rewrite_report.json/csv`。

修订痕迹版规则：

- 如果用户上传了第一版/原始 Word，则修订版比较“第一版/原始 Word -> 最终返修版”。
- 如果用户没有上传第一版/原始 Word，则修订版比较“当前待返修 Word -> 最终返修版”。
...
注意：检测报告必须尽量对应同一版 Word。若报告来自旧版文档而 Word 是新生成版，匹配数量会明显下降。
...
- 随后根据用户要求，默认英文降 AI 模板替换为“专业 AIGC 降率工具”提示词：专名多则压缩，专名少则适度扩展，可适当口语化但保持专业性，重点是降低 AIGC 率。
- 用户用修正版生成 `润色版_高楠-论文 4.docx` 和 `rewrite_report (1).json` 后确认：题名、Keywords、关键词、原创性声明、致谢已被保护，没有再被扩写成正文。
- 但报告显示 `117` 段进入队列，`88` 段 changed，`28` 段 failed，`1` 段 unchanged。失败原因当时统一显示为“疑似短标题或标签被扩写成正文”，不便诊断。
- 最新修复已新增报告字段：`rewrite_rounds`、`second_rewrite_applied`、`reject_reason`，用于确认二次改写是否执行，以及安全阀具体为什么拒绝写回。
- 最新修复还会在扫描到 `Reference` / `References` / `Bibliography` / `参考文献` 后跳过后续参考文献条目，避免参考文献被送入 AI。
...
- AIGC 检测报告定点返修模式：上传报告、提取疑似片段、匹配 Word 段落、只返修命中段落。
- 定点返修支持可选上传第一版/原始 Word，用于生成相对原文的修订痕迹版。
- 适配 PaperYY 新版 `AIGC检测报告.html`：可从 `em.high`、`em.medium`、`em.low` 标记中提取疑似 AI 片段，供定点返修匹配 Word 段落。
- 修订痕迹版改为可选生成，默认关闭。
- 新增独立“生成修订文档”模式，可上传两份 Word 并用 `docx_editor` 生成修订版。
- 新增中文中转回译开关，支持每段先翻译成中文再翻译回英文。
- 格式安全阀改为可选，默认开启；关闭时仅跳过 `markdown_formatting_added` 拦截。
...
- 参考文献章节必须整体跳过。单条参考文献很长，容易被误判为正文；如果送进 AI，既浪费 token，又会破坏引用格式。

### 5. 二次改写需要可观测

用户曾怀疑多线程下二次改写结果被丢。代码路径上，二次改写是在每个 future 返回后、写回前同步执行，通常不是“被并发丢失”；真正的问题是旧报告没有记录二次改写次数和是否应用。

经验：

- `rewrite_report` 必须保留 `rewrite_rounds` 和 `second_rewrite_applied`。
- 安全阀拒绝写回时，必须保留 `reject_reason`，不要只给笼统错误。
- 否则无法区分：二次改写没触发、二次改写触发但仍被安全阀拒绝、或二次改写成功但残留风险。
...
- 用同一版 Word 和对应 AIGC 报告测试“检测报告定点返修”。
- 重点观察 `rewrite_report` 里的 `match_score`、`matched_fragments`、`rewrite_rounds`、`reject_reason`。
- 若匹配数过少，优先改进报告解析和匹配逻辑，而不是先改 prompt。
- 若返修后检测仍偏高，再基于报告命中片段继续细化章节规则。
